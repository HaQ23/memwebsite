<?php
	...
	if(!isset($_GET['id'])) {
		...
		$idMeme = $_GET['id'];
		$sql = "SELECT id_user FROM meme WHERE id_meme = $idMeme";
		...
		if($result->num_rows > 0) {
			...
			header("Location: ./index.php?error=memeid");
			exit();
		}
		if($authorId != 9999) {
			$sql = "SELECT imgsource, account.name, account.surrname, adding_date FROM meme INNER JOIN account ON meme.id_user = account.id_user WHERE id_meme = '$idMeme'";
			$result = $conn->query($sql);
	
			if($result->num_rows > 0) {
				while($row = $result->fetch_assoc()) {
					$authorName = $row['name'].' '.$row['surrname'];
					$addingDate = $row['adding_date'];
					$memeUrl = $row['imgsource'];
				}
			}
		} else {
			$sql = "SELECT imgsource,adding_date FROM meme WHERE id_meme = '$idMeme'";
			$result = $conn->query($sql);
...
<body>
    ...
    <main class="main box">
        <img src="./memes/<?php echo $memeUrl; ?>" alt="" class="meme__image">
        <div class="meme__info">
            <p class="meme__author">Dodał: <span class="meme__date--highlighted"><?php echo $authorName; ?></span></p>
            <p class="meme__date">Data dodania: <span class="meme__date--highlighted"><?php echo $addingDate; ?></span></p>
        </div>
    </main>
    <footer class="footer">
	<div class="wrapper">
